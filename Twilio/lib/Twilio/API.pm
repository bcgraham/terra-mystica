...
Twilio::API - Accessing Twilio's REST API with Perl

=head1 SYNOPSIS

  use Twilio::API;

  my $twilio = Twilio::API->new(AccountSid => 'AC12345...',
                                     AuthToken  => '1234567...');
...
This section is for existing B<Twilio::API> users considering an
upgrade from B<Twilio::API> versions prior to 0.15. If you're new
to B<Twilio::API> you may safely unconcern yourself with this
section and move on to L</DESCRIPTION> below.

B<Twilio::API> since version 0.15 defaults to Twilio's
...
to explicitly use I<API_VERSION> as '2008-08-01', like this:
  ...
  my $twilio = Twilio::API->new
    ( AccountSid  => 'AC12345...',
      AuthToken   => '1234567...',
      API_VERSION => '2008-08-01' );  ## <-- add this line here

Now you may safely upgrade B<Twilio::API> and stay current. You
...
Now you can safely upgrade B<Twilio::API>.

=back

=head1 DESCRIPTION

B<Twilio::API> aims to make connecting to and making REST calls
on the Twilio API easy, reliable, and enjoyable.

You should have ready access to Twilio's API documentation in order to
use B<Twilio::API>.

B<Twilio::API> knows almost nothing about the Twilio API itself
...
API documentation and translate it into B<Twilio::API> calls.
...
B<Twilio::API>, but here's a teaser to help you see how easy your
...
B<Twilio::API> makes requests and handles responses for you, so
...
=head2 Using Twilio::API

Now that we have a basic understanding of how Twilio's REST API works,
we can translate the API into B<Twilio::API> method calls. Doing
this is trivial:
...
Create a B<Twilio::API> object and make the call using the I<API
...
Here are the examples:
  ...
  ## create an object
  my $twilio = new Twilio::API( AccountSid => '{your account sid}',
...
NOTE: B<Twilio::API> prior to version 0.15 defaulted to
'2008-08-01'; if you're upgrading B<Twilio::API>, see
...
B<Twilio::API> handles that for you (this just means that you
...
Versions of B<Twilio::API> prior to 0.15 defaulted to
F<2008-08-01>. API calls since B<Twilio::API> version 0.15 and
